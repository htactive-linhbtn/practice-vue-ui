...
<div>
      ...
      <tbody v-for="(staff,index) in listStaffs" :key="index" class="text-center">
        <tr>
          ...
          <td>
            ...
            <button>
              <router-link v-bind:to="'/detail/'+staff.id">
                Xem Chi Tiết
              </router-link>
            </button>
          </td>
  ...
  <div>
    <b-modal ref="modal-0" hide-footer title="Edit  ">
      <div class="d-block text-center">
        <div class="">
          <div>
      ...
      <b-button class="mt-2 " variant="outline-warning" block @click="okEdit()">Ok</b-button>
    </b-modal>
  </div>
</div>
</template>
...
export default {
  ...
  methods: {
    ...
    okEdit() {
      this.tempIndex[this.editObj] = this.listStaffs[this.newObj];
    },
